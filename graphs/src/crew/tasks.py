"""
This module contains the tasks for the crew graph.
"""
...
class EmailFilterTasks:
    def filter_emails_task(self, agent, emails):
		"""
		Filter out non-essential emails such as newsletters, promotional content and notifications.
		"""
        return Task(
            description=dedent(
                f"""\
    ...
    def action_required_emails_task(self, agent):
		"""
		Identify emails that require action from the user.
		"""
        return Task(
            description=dedent(
                """\
